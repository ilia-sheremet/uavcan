...
TEST(CanAcceptanceFilter, Basic_test)
{
    ...
    {
        std::cout<<"config.ID ["<<i<<"]= "<< std::bitset<29>(configure_array.getByIndex(i)->value.id)<<std::endl;
        std::cout<<"config.MK ["<<i<<"]= "<< std::bitset<29>(configure_array.getByIndex(i)->value.mask)<<std::endl;
    }

    ASSERT_EQ(configure_array.getByIndex(0)->value.id, 363069440);
    ASSERT_EQ(configure_array.getByIndex(0)->value.mask, 536739840);
    ASSERT_EQ(configure_array.getByIndex(1)->value.id, 268435456);
    ASSERT_EQ(configure_array.getByIndex(1)->value.mask, 469762048);
    ASSERT_EQ(configure_array.getByIndex(2)->value.id, 16777216);
    ASSERT_EQ(configure_array.getByIndex(2)->value.mask, 124452864);
    ASSERT_EQ(configure_array.getByIndex(3)->value.id, 18874368);
    ASSERT_EQ(configure_array.getByIndex(3)->value.mask, 133169152);
}
#endif
